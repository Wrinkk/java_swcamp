...
public class Car {
    ...
    /* 설명. 시동이 걸려있는 상태인 경우에는 할 일이 없고, 시동이 걸려있지 않은 상태면 시동을 건다. */
    public void startUp() {
        if (isOn) {          //시동이 걸려 있으면,
    ...
    /* 설명. 시동이 걸려있는 상태인 경우 시동을 끄고, 이미 꺼진 상태라면 끌 수 있고 주행중이라면 끌 수없다.*/
    public void turnoff() {
        if (isOn) {
            if (speed > 0) {
                System.out.println("달리는 상태에서 시동을 끌 수 업습니다. 차를 우선 멈춰주세요.");
            } else {
                isOn = false;
                System.out.println("시동을 끕니다. 다시 운행하시려면 시동을 켜 주세요! Bye~");
            }
        } else {
            System.out.println("이미 시동이 꺼져 있는 상태입니다. 시동 상태를 확인 해주세요.");
        }
    }
}
