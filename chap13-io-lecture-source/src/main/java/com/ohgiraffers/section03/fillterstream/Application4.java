...
import com.ohgiraffers.section03.fillterstream.DTO.MemberDTO;
...
public class Application4 {
    ...
    public static void main(String[] args) {
        ...
        /* 수업목표. 객체단위 입출력 보조스트링믈 이해하고 활용할 수 있다. */
        ...
        File file = new File("src/main/java/com/ohgiraffers/section03/fillterstream/testObject.txt");
        ...
        try {
            // outputstream 상속 계열들만 올수있음
            if (!file.exists()) { // 해당 위치에 파일이 존재하지 않는다면(처음)
                objOut = new ObjectOutputStream(
                        new BufferedOutputStream(
                                new FileOutputStream("src/main/java/com/ohgiraffers/section03/fillterstream/testObject.txt")));

            } else {                // 해당 위치에 파일이 존재한다면(처음 이후)
                objOut = new MyOutput(
                        new BufferedOutputStream(
                                new FileOutputStream("src/main/java/com/ohgiraffers/section03/fillterstream/testObject.txt", true)));
            }
...
// outputstream 상속 계열들만 올수있음
            ois = new ObjectInputStream(
                    new BufferedInputStream(
                            new FileInputStream("src/main/java/com/ohgiraffers/section03/fillterstream/testObject.txt")));
            ...
            System.out.println("객체 단위 파일 입력완료");
        } catch (IOException e) {
            throw new RuntimeException(e);
