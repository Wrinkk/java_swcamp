...
//        ArrayList<Objects> aList = new ArrayList<>();         // 다이아몬드 연산자가 없는 제네릭 클래스는 <Object>와 같다.
...
//            for (int i = 2; i < newArr.length - 1; i++) {         // 기존의 값을 뒤로 옮길 때 앞의 인덱스 부터 옮기면 안됨.
            newArr[i + 1] = newArr[i];
        }
        System.out.println(Arrays.toString(newArr));
        ...
        stringList = new LinkedList<>(stringList); // 다른거에 비해 내림차순으로 바꿀때 편함.
