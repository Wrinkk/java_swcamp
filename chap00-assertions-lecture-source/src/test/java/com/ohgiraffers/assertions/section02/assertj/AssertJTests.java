...
public class AssertJTests {
    ...
    /* 목차. 1. 문자열 테스트하기*/
    /* 필기. 문자열의 패턴들로 검증하기 때문에 일치성 여부만 판단하는 것보다 다양한 테스틑를 할 수 있다. */
    ...
    /* 목차. 3. 날짜 테스트하기 */
    /* 필기. 날짜 객체가 가지는 년,월,일,시,분,초,밀리초,나노초까지 검증이 가능하며, 특정 날자 범위, 이전, 이후 내용도 검증 가능하다. */
    @Test
    ...
    /* 목차. 4. 예외 테스트하기*/
    /* 필기. 예외 내용을 검증하는 것 또한 예외 타입, 예외 메시지 등 검증할 수 있다. */
    ...
    /* 필기. 특정 filter를 자바 람다식을 이용하여 표현할 수 있는 유용한 기능읻. */
    @Test
    ...
    /* 목차. 6. 객체 프로퍼티 검증하기*/
    /* 필기. 개체의 프로퍼티를 추출하여 filter를 할 수 있다. 이때 getter함수가 존재하지 않아도 프로퍼티 필터링이 가능하다. */
    @Test
