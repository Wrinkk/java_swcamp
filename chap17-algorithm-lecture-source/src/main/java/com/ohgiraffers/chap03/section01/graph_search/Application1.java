...
public class Application1 {
    ...
    public static Integer solution(String input) throws IOException {
        ...
        map = new int[node + 1][node + 1];      // node와 번호와 인덱스 위치를 일치시키키 위해서 크기를 1로 증가)0번 인덱스 사용 X)

        /* 설명. 방문 배열 작성(방문 배열을 작성하는 이유는 방문했던 노드를 다시 stack에 담지 않거나 다시 재귀 함수 호출을 하지 않기 위함) */
        visit = new boolean[node + 1];          // 방문 배열도 노드 번호와 인덱스 번호를 이치 시키기 위해서 1 증가
        ...
        for (int i = 0; i < edge; i++) {
            ...
            /* 설명. 무방향 그래프로 처리되기 위해 노드 번호들(인덱스 번호)을 반대로 적용한다. */
//            map[a][b] = 1;
//            map[b][a] = 1;
            map[a][b] =map[b][a] = 1;
        }
