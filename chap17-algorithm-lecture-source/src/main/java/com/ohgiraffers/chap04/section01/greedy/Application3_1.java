...
public class Application3_1 {
    ...
    public static int solution(String input) throws IOException {
        ...
        /* 설명. 그리디 알고리즘으로 접근하자면 하나의 회의실에서 가장 많은 회의를 열기 위해서는
            종료시간 순(오름차순)으로 고민하는 것이 좋다. (빨리 끝나는 회의를 먼저 고려) */
        Arrays.sort(time, (o1, o2) -> {
            ...
            /* 설명. 종료시간이 같지 않을 떄는 종료 시간이 이른 회의부터 정렬되도록 기준을 잡는다.(종료시간 오름차순) */
            return o1[1] - o2[1];
        });
        ...
        /* 설명. time 배열에 담긴 회의를 확인하며, 앞선 호의의로 종료 다음 회의가 진행되는지를 판별해 가능한 회의 추출 */
        for (int i = 0; i < N; i++) {

            /* 설명. 앞선 회의가 끝나는 시간과 일치하거나 이후에 시작하는 회의 인가*/
            if(end <=time[i][0]){

                /* 설명. 조건을 만족하는 종료 시간을 end변수를 갱신*/
                /* 설명. 조건을 만족하는 회의의 종료 시간으로 end 변수 갱신 */
                end = time[i][1];
                max_count++;
